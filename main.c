...
"    }\n"
"    if (projMode == 2) {\n"
"        // Sphere: par_shapes parametric sphere has v=0 at the north pole and\n"
"        // v=1 at the south pole, which already matches the image (north on\n"
"        // top), so no vertical flip is applied here.\n"
"    } else {\n"
...
static bool Button(Rectangle r, const char *text, bool active)
{
    ...
    Color bg;
    if (active)      bg = (Color){ 70, 130, 180, 255 };
    else if (hover)  bg = (Color){ 95, 95, 95, 255 };
    ...
    DrawText(text, (int)(r.x + (r.width - fw)/2), (int)(r.y + (r.height - fs)/2), fs, RAYWHITE);
...
int main(int argc, char **argv)
{
    ...
    while (!WindowShouldClose()) {
        ...
        float bw = 180.0f, bh = 32.0f, by = (BAR_HEIGHT - bh)/2.0f, gap = 8.0f, bx = 8.0f;
        if (Button((Rectangle){ bx, by, bw, bh }, "Equirectangular", projMode == PROJ_EQUIRECT))
            projMode = PROJ_EQUIRECT;
        bx += bw + gap;
        if (Button((Rectangle){ bx, by, 120, bh }, "Mercator", projMode == PROJ_MERCATOR))
            projMode = PROJ_MERCATOR;
        bx += 120 + gap;
        if (Button((Rectangle){ bx, by, 110, bh }, "Sphere", projMode == PROJ_SPHERE))
            projMode = PROJ_SPHERE;

        DrawText("drag: rotate   shift+drag: pan   wheel: zoom",
